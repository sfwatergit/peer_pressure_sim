 ...
 *                                                                         *
 * *********************************************************************** */
package sandbox.sfwatergit.analysis.modules.ptAccessibility;
...
import sandbox.sfwatergit.analysis.modules.ptAccessibility.stops.Circle;
import sandbox.sfwatergit.analysis.modules.ptAccessibility.stops.Mode2StopMap;
import sandbox.sfwatergit.analysis.modules.ptAccessibility.stops.PtStopMap;
import sandbox.sfwatergit.analysis.modules.ptAccessibility.utils.DistCluster2ActCnt;
import sandbox.sfwatergit.analysis.modules.ptAccessibility.utils.PtAccesShapeWriter;
import sandbox.sfwatergit.analysis.modules.ptAccessibility.utils.PtAccessMapShapeWriter;
...
public class PtAccessibility extends AbstractAnalysisModule {
    ...
    public PtAccessibility(Scenario sc, List<Integer> distanceCluster, int quadrantSegments, SortedMap<String, List<String>> activityCluster, String targetCoordinateSystem, int gridSize) {
        ...
        this.activityCluster.put("unknown", new ArrayList<>());
        this.targetCoordinateSystem = targetCoordinateSystem;
        this.gridSize = gridSize;
    }

    /**
     * @param quadrantSegments segments to split cluster up into
     * @param distances        distances at which to create clusters
     * @return map of cluster radii to {@link Circle} objects
     */
    ...
    public void preProcessData() {
        ...
        for (Person p : this.scenario.getPopulation().getPersons().values()) {
            p.getSelectedPlan().getPlanElements().stream().filter(
                    pe -> pe instanceof Activity).forEach(
                    pe -> locationMap.addActivity((Activity) pe, p.getId()));
        }
    }
    ...
    public void postProcessData() {
        this.mode2DistanceCluster2ActCnt = new TreeMap<>();
        // iterate over all ptStopMaps (per mode)
        for (final Entry<String, PtStopMap> mode2Stops : this.mode2Stop.getStopMaps().entrySet()) {
            final DistCluster2ActCnt cnt = new DistCluster2ActCnt(this.distanceCluster.keySet(), this.activityCluster.keySet());
            // count the number of activities per Type
            for (final Entry<String, List<ActivityLocation>> type2locations : this.locationMap.getType2Locations().entrySet()) {
                for (final ActivityLocation l : type2locations.getValue()) {
                    // increase when the activity takes place within this distance
                    this.distanceCluster.keySet().stream().filter(
                            dist -> mode2Stops.getValue().contains(l.getCoord(), dist)).forEach(
                            dist -> cnt.increase(dist, type2locations.getKey()));
                }
            }
...
//			PtAccesShapeWriter.writeMultiPolygons(m.getDecision().getCluster(), outputFolder + m.getKey() + PtStopMap.FILESUFFIX, m.getKey());
            for (Entry<String, MultiPolygon> e : m.getValue().getCluster().entrySet()) {
                if (!cluster2mode2area.containsKey(e.getKey())) {
                    cluster2mode2area.put(e.getKey(), new HashMap<>());
                }
        ...
        for (Entry<String, DistCluster2ActCnt> e : this.mode2DistanceCluster2ActCnt.entrySet()) {
            writer = IOUtils.getBufferedWriter(outputFolder + e.getKey() + ".csv");
            try {
                //toFile the header
                ...
                for (Entry<String, SortedMap<String, Double>> ee : e.getValue().getResults().entrySet()) {
                    ...
                    // get the counts per actType and toFile the relative value
