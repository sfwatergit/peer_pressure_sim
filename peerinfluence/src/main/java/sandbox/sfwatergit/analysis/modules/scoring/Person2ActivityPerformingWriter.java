...
import sandbox.sfwatergit.analysis.filters.PersonFilter;
import sandbox.sfwatergit.analysis.modules.UserGroup;
...
public class Person2ActivityPerformingWriter {
    ...
    public static void main(String[] args) {
        new Person2ActivityPerformingWriter().run(args[0]);
    }

    private void run(String eventsFile) {

        ActivityType2ActDurationsAnalyzer actAnalyzer = new ActivityType2ActDurationsAnalyzer(outputFilesDir,eventsFile);
        ...
        for (Id<Person> personId : personId2Act2EndTimes.keySet()) {
            ...
            for (int index = 1; index < actEndTimes.size() - 1; index++) {
                if (!actStartTimes.get(index).getFirst().equals(actEndTimes.get(index).getFirst()))
                    throw new RuntimeException("Activities are not same. Aborting...");//just for double check
        ...
        //finally toFile the data
        writeActType2UtilPerforming(outputFilesDir + "/travelTime/");
    }
    ...
    private void storeUtilOfPerforming(final Scenario sc, Person p, final Activity activity) {
        ScoringFunctionFactory sfFactory = new ScoringFunctionFactory() {
            CharyparNagelScoringParameters.Builder params = new CharyparNagelScoringParameters.Builder(sc, p.getId());

            @Override
            public ScoringFunction createNewScoringFunction(Person person) {
                Person2ActivityScoringFunction scrFn = new Person2ActivityScoringFunction(new CharyparNagelActivityScoring(params.build()));
                return scrFn;
            }
        };
