 ...
 *                                                                         *
 * *********************************************************************** */
package sandbox.sfwatergit.analysis.modules.userbenefits;
...
public class UserBenefitsCalculator {
	...
	private final Map<Id<Person>, Double> personId2Utility = new HashMap<>();
	private final Map<Id<Person>, Double> personId2MonetizedUtility = new HashMap<>();
	...
	public double calculateUtilityOfPerson_utils(Person person) {
		...
		if(this.welfareMeasure.equals(WelfareMeasure.LOGSUM)){
			for(Plan plan : person.getPlans()) {
				...
				shouldBeConsidered = this.considerAllPlans || testScore(plan, person.getId());
...
//					double expScoreOfPlan = Math.exp(betaLogit * plan.getScore());
			...
			boolean shouldBeConsidered;
			shouldBeConsidered = this.considerAllPlans || testScore(selectedPlan, person.getId());
			if (shouldBeConsidered) {
	...
	public int getInvalidPlans() {
		if (this.considerAllPlans) {
			...
			return this.minusScore + this.nullScore;
		}
	}
