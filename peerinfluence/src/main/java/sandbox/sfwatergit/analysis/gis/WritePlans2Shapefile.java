...
class SelectedPlans2ESRIShape {
    ...
    private SimpleFeature getActFeature(final String id, final Activity act) {
        ...
        CoordinateTransformation ct = TransformationFactory.getCoordinateTransformation("epsg:26910", "epsg:3857");
        Coord c = ct.transform(act.getCoord());
        final String mode = PlanUtils.getModeOfSelectedPlan(population.getPersons().get(Id.createPersonId(id)));

        try {
            return this.actBuilder.buildFeature(null, new Object [] {MGC.coord2Point(c),mode,   id, type, linkId, startTime, endTime});
    ...
    private void initFeatureType() {
        ...
        actBuilder.add("MODE",String.class);
