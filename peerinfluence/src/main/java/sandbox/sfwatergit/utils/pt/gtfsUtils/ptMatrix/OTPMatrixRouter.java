...
//        try {
//
//            Graph graph = Graph.load(new FileInputStream(inputRoot + "/Graph.obj"), Graph.LoadLevel.FULL);
...
//            CoordinateTransformation ct2 = TransformationFactory.getCoordinateTransformation("EPSG:26910", "EPSG:4326");
//            Coord inputCoord = CoordUtils.createCoord(individual.lon, individual.lat);
//            Coord outputCoord = ct2.transform(inputCoord);
//            Sample sample = sampleFactory.getSample(outputCoord.getX(), outputCoord.getY());
//            if (sample == null) {
//                counter++;
...
//        Coord inputCoord = CoordUtils.createCoord(individual.lon, individual.lat);
//        Coord outputCoord = ct2.transform(inputCoord);
//        request.from = new GenericLocation(outputCoord.getY(), outputCoord.getX());
//
//        try {
...
//    }
//
//    static CompletableFuture<RoutingRequest> getRoutingRequestAsync(Graph graph, Calendar cal, Individual ind) {
//        return CompletableFuture.supplyAsync(() -> getRoutingRequest(graph, cal, ind));
//    }
//
//    static CompletableFuture<List<State>> evalAsync(ShortestPathTree spt, Sample sample){
//        return CompletableFuture.supplyAsync(() -> eval(spt, sample));
//    }
//
//    static CompletableFuture<ShortestPathTree> getSptAsync(RoutingRequest request){
//        return CompletableFuture.supplyAsync(() -> (new AStar()).getShortestPathTree(request));
//    }
//
//
//    static CompletableFuture<Void> routeAsync(Individual toIndividual, CompletableFuture<ShortestPathTree> spt, InputsCSVWriter timeWriter, InputsCSVWriter distanceWriter){
//        ShortestPathTree shortestPathTree = spt.getNow(new ShortestPathTree(new RoutingRequest(), new DominanceFunction.EarliestArrival()));
//        return CompletableFuture.runAsync(() -> route(toIndividual, shortestPathTree, timeWriter, distanceWriter));
//    }
//
//
//
//    private static void routeMatrixAsync(Graph graph, Calendar calendar, List<Individual> fromIndividuals, List<Individual> toIndividuals, String outputDir) {
...
//
//        fromIndividuals.forEach(fi -> toIndividuals.stream()
//                .map(ti -> routeAsync(ti, getRoutingRequestAsync(graph, calendar, fi)
//                        .thenComposeAsync(OTPMatrixRouter::getSptAsync), timeWriter, distanceWriter)));
//
//    }
//
