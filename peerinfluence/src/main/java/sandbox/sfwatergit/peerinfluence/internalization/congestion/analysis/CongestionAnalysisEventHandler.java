package sandbox.sfwatergit.peerinfluence.internalization.congestion.analysis;/* *********************************************************************** *
 * project: org.matsim.*
...
public class CongestionAnalysisEventHandler implements PersonMoneyEventHandler, TransitDriverStartsEventHandler, ActivityEndEventHandler, PersonDepartureEventHandler, LinkEnterEventHandler, PersonEntersVehicleEventHandler, PersonLeavesVehicleEventHandler, CongestionEventHandler {
    ...
    // This travelTime uses either money events or congestion events.
    ...
    private Map<Id<Person>, Double> causingAgentId2amountSum = new HashMap<>();
    private Map<Id<Person>, Double> affectedAgentId2amountSum = new HashMap<>();
    private Set<Id<Person>> persons = new HashSet<>();

    // for pt-distance calculation
    private Map<Id<Person>, Double> personId2distanceEnterValue = new HashMap<>();

    private List<Id<Person>> ptDrivers = new ArrayList<>();
    ...
    public CongestionAnalysisEventHandler(Scenario scenario, boolean useMoneyEvents) {
        ...
        if (useMoneyEvents) {
            log.warn("Money events may be thrown later than the congestion events... May result in a wrong interpretation of the results. Better use directly the congestion events for travelTime.");
        }
    ...
    public void handleEvent(CongestionEvent event) {
        ...
        if (!useMoneyEvents) {
            ...
            // person-based travelTime - causing agent
            ...
            // person-based travelTime - affected agent
...
//	// ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ...
    public Map<Id<Person>, List<Double>> getPersonId2listOfDepartureTimes(String mode) {
        ...
        for (Id<Person> personId : personId2tripNumber2departureTime.keySet()) {
            ...
            for (int i : personId2tripNumber2departureTime.get(personId).keySet()) {
                if (personId2tripNumber2legMode.get(personId).get(i).equals(mode)) {
    ...
    public Map<Double, Double> getAvgAmountPerTripDepartureTime(String mode) {
        ...
        double timeBinSize = 900.0;
        double startTime = timeBinSize;
        double periodLength = timeBinSize;
        ...
        Map<Integer, double[]> counter2allDepartureTimesAndAmounts = new HashMap<>();
        int i = 0;

        for (Id<Person> personId : personId2tripNumber2departureTime.keySet()) {
            for (int tripNumber : personId2tripNumber2departureTime.get(personId).keySet()) {
                if (personId2tripNumber2legMode.get(personId).get(tripNumber).equals(mode)) {
        ...
        if (ptDrivers.contains(event.getPersonId())) {
            ...
            if (personId2tripNumber2legMode.containsKey(event.getPersonId())) {
                ...
                if (tripNumber2legMode.containsKey(tripNumber)) {
                    ...
                    if (tripNumber2legMode.get(tripNumber).equals("pt")) {
                    } else {
    ...
    public Map<Id<Person>, Double> getCausingAgentId2amountSumAllAgents() {
        ...
        if (this.scenario.getPopulation().getPersons().isEmpty()) {
            log.warn("Scenario does not contain a Population. Using the person IDs from the events file for the person-based travelTime (total: " + personIds.size() + ").");
            personIds.addAll(this.persons);
        } else {
            log.info("Scenario contains a Population. Using the person IDs from the population for the person-based travelTime.");
    ...
    public Map<Id<Person>, Double> getAffectedAgentId2amountSumAllAgents() {
        Map<Id<Person>, Double> personId2amountSumAllAgents = new HashMap<>();
