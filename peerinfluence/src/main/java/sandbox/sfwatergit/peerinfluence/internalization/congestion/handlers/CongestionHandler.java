...
import sandbox.sfwatergit.peerinfluence.internalization.congestion.LinkCongestionInfo;
...
public abstract class CongestionHandler implements
    ...
    final List<Id<Vehicle>> ptVehicleIDs = new ArrayList<>();
    ...
    public CongestionHandler(EventsManager events, Scenario scenario) {
        this.events = events;
    ...
    public void handleEvent(PersonStuckEvent event) {
        log.warn("An agent is stucking. No garantee for right calculation of routeCaching congestion effects: " + event.toString());
    }
...
//					log.warn("The causing agent and the affected agent are the same (" + id.toString() + "). This situation is NOT considered as an routeCaching effect; NO marginal congestion event is thrown.");
                } else {
